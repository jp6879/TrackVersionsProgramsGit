...
function Test_P()
    ...
    Ns = [1000000, 1000000, 1000000, 1000000, 1000000, 1000000]
    pl = plot(xlabel="l (μm?)", ylabel="P(l)", legend=:best)
    xlims!(0, 6)
    for i in 1:length(μs)
        p = P(μs[i], σs[i], Ns[i])
        kde_result = kde(p)
        kde_result.x, kde_result.density
        plot!(pl,kde_result.x, kde_result.density, label = L"$μ$" * "= $(μs[i])" * L" $σ$" * "= $(σs[i])")
        #histogram!(pl, p, label=L"$\mu$" * "= $(μs[i])" * L"$\sigma$" * "= $(σs[i])", )
    end
    pl
end
...
function S_han(lc, N, time_sim)
    ...
    pl = plot(xlabel="t (s)", ylabel="Señal", legend=:best)
    xlims!(0, 0.15)
    S = zeros(length(time))
    M = zeros(length(time))
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "8c86b3748f42fb266b8ae28059dba3f99b53a7c1"
