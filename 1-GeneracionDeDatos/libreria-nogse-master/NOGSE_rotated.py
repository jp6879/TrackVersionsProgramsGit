...
class Magnetization:
	...
	def LogNormal(self,l,x1,x2):
		lc=x1*np.cos(self.angle)-x2*np.sin(self.angle)
		Sigma=x1*np.sin(self.angle)+x2*np.cos(self.angle)
...
class Distribution_Signal(Magnetization):
	def __init__(self,D0,sizes,angle): #los tamaños se suponen equiespaciados E IMPARES
		super().__init__(D0,angle)
		self.sizes = sizes
		self.weights = np.ones(sizes.shape) #pesos para la integracion por simpson
		if(sizes.shape[0]%2==0):
			print("La cantidad de tamaños debe ser par")
